...
public class TellAction extends RepeatGuardedAction {
    ...
    protected ActionResult applyNonRepeated(BlacklistEvent event, boolean silent) {
        ...
        if (event.getPlayer() != null) {
            if (message != null) {
                event.getPlayer().printRaw(ChatColor.RED + String.format(message, event.getTarget().getFriendlyName()));
            } else {
                event.getPlayer().printRaw(ChatColor.RED + "Вам не разрешено " + event.getDescription() + " "/* + event.getTarget().getFriendlyName() */+ "этот предмет.");
            }
        }
