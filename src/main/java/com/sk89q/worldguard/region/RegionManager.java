...
public class RegionManager {
     ...
     */
    private ConcurrentHashMap<String, RegionIndex> managers;
     ...
     */
    public RegionManager(WorldGuardPlugin plugin) {
        this.plugin = plugin;
        config = plugin.getGlobalStateManager();
        managers = new ConcurrentHashMap<String, RegionIndex>();
    ...
    public void unload(String name) {
        RegionIndex manager = managers.remove(name);
    ...
    public RegionIndex load(World world) {
        RegionIndex manager = create(world);
     ...
     */
    public RegionIndex create(World world) {
        String name = world.getName();
        boolean sql = config.useSqlDatabase;
        RegionStore database;
        File file = null;

        try {
            if (!sql) {
                file = getPath(name);
                database = new YamlStore(file, plugin.getLogger());
            ...
            // Create a manager
            RegionIndex manager = new PriorityRTreeIndex(database);
            manager.load();
     ...
     */
    public RegionIndex get(World world) {
        RegionIndex manager = managers.get(world.getName());
        RegionIndex newManager = null;
     ...
     *
     * @see #allows(com.sk89q.worldguard.region.flags.StateFlag, org.bukkit.Location, com.sk89q.worldguard.LocalPlayer)
     ...
     * @param flag The flag to check
     * @param loc The location to check the flag at
     * @param player The player to check for the flag's {@link com.sk89q.worldguard.region.flags.RegionGroup}
     * @return Whether the flag is allowed
     */
