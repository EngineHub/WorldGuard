        ...
        synchronized (lock) {
            for (Flag<?> flag : flags) {
                try {
                    ...
                    log.log(Level.WARNING, message("flags.registry.register-all-conflict", e.getMessage()));
                }
            }
        }
    }
    ...
    private Flag<?> forceRegister(Flag<?> flag) throws FlagConflictException {
        ...
        synchronized (lock) {
            ...
            if (flags.containsKey(name)) {
                throw new FlagConflictException(message("flags.registry.conflict", name));
            }
            ...
            if (flag != null) {
                try {
                    ...
                    if (unmarshalled != null) {
                        ...
                        log.warning(message("flags.registry.parse-fail", flag.getName(), entry.getValue()));
                    }
                } catch (Throwable e) {
                    log.log(Level.WARNING, message("flags.registry.unmarshal-fail", flag.getName()), e);
                }
            }
        }
