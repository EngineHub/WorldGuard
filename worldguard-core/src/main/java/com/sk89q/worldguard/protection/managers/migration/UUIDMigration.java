...
public class UUIDMigration extends AbstractMigration {
    ...
    protected void migrate(RegionDatabase store) throws MigrationException {
        log.log(Level.INFO, message("migration.uuid.start", store.getName()));
        ...
        try {
            ...
            throw new MigrationException(message("migration.uuid.load-fail", store.getName()), e);
        }
        ...
        try {
            ...
            throw new MigrationException(message("migration.uuid.save-fail", store.getName()), e);
        }
    }
    ...
    private boolean migrate(Collection<ProtectedRegion> regions) throws MigrationException {
        ...
        if (!names.isEmpty()) {
            ...
            try {
                ...
                log.log(Level.INFO, message("migration.uuid.resolving", names.size()));
                ...
                throw new MigrationException(message("migration.uuid.service-fail"), e);
            } catch (InterruptedException e) {
                throw new MigrationException(message("migration.uuid.interrupted"));
            } finally {
            ...
            log.log(Level.INFO, message("migration.uuid.converted"));
            convert(regions);
    ...
    protected void postMigration() {
        if (!unresolvedNames.isEmpty()) {
            if (keepUnresolvedNames) {
                log.log(Level.WARNING, message("migration.uuid.unresolved-keep"));
            } else {
                log.log(Level.WARNING, message("migration.uuid.unresolved-removed"));
            }
        }
    }
