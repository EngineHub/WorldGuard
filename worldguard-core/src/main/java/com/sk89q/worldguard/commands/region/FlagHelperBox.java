...
class FlagHelperBox extends PaginationBox {
    ...
    FlagHelperBox(World world, ProtectedRegion region, RegionPermissionModel perms) {
        super(message("commands.region.flags.helper.title", region.getId()),
                String.format(Locale.ROOT, "/rg flags -w \"%s\" -p %%page%% %s", world.getName(), region.getId()));
        this.world = world;
        this.region = region;
        this.perms = perms;
    }
    ...
    public Component getComponent(int number) {
        if (number == Flags.INBUILT_FLAGS.size()) {
            return centerAndBorder(TextComponent.of(message("commands.region.flags.helper.third-party"), TextColor.AQUA));
        ...
        if (flag.usesMembershipAsDefault()) {
            builder.append(TextComponent.empty().append(TextComponent.of("*", TextColor.AQUA))
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of(message("commands.region.flags.helper.member-default")))));
            length += monoSpace ? 1 : FlagFontInfo.getPxLength('*');
        }
        if (flag == Flags.PASSTHROUGH) {
            builder.append(TextComponent.empty().append(TextComponent.of("*", TextColor.AQUA))
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of(message("commands.region.flags.helper.build-override")))));
            length += monoSpace ? 1 : FlagFontInfo.getPxLength('*');
        }
    ...
    private <V> void appendValueChoices(TextComponent.Builder builder, Flag<V> flag, Iterator<V> choices, @Nullable String suggestChoice) {
        ...
        while (choices.hasNext()) {
            ...
            if (maySet) {
                if (isExplicitSet) {
                    hoverTexts.add(TextComponent.of(message("commands.region.flags.helper.unset"), TextColor.GOLD));
                } else if (DANGER_ZONE.contains(flag) && !(ProtectedRegion.GLOBAL_REGION.equals(region.getId()) && flag == Flags.PASSTHROUGH)) {
                    hoverTexts.add(TextComponent.of(message("commands.region.flags.helper.danger.line1"), TextColor.RED)
                            .append(TextComponent.newline())
                            .append(TextComponent.of(message("commands.region.flags.helper.danger.line2"))
                            .append(TextComponent.newline())
                            .append(TextComponent.of(message("commands.region.flags.helper.danger.line3")))));
                } else {
                    hoverTexts.add(TextComponent.of(message("commands.region.flags.helper.set"), TextColor.GOLD));
                }
            }
        ...
        if (suggestChoice != null && perms.maySetFlag(region, flag)) {
            builder.append(TextComponent.of(suggestChoice, TextColor.DARK_GRAY)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of(message("commands.region.flags.helper.custom"), TextColor.GOLD)))
    ...
    private <V> void appendValueText(TextComponent.Builder builder, Flag<V> flag, String display, @Nullable Component hover) {
        ...
        if (maySet) {
            if (isExplicitSet) {
                hoverTexts.add(TextComponent.of(message("commands.region.flags.helper.change"), TextColor.GOLD));
            } else {
                hoverTexts.add(TextComponent.of(message("commands.region.flags.helper.set"), TextColor.GOLD));
            }
        }
    ...
    private String makeCommand(Flag<?> flag, Object choice) {
        return String.format(Locale.ROOT, "/rg flag -w \"%s\" -h %d %s %s %s",
                world.getName(), getCurrentPage(), region.getId(), flag.getName(), choice);
    }
    ...
    private <V> Component getToolTipHint(V defVal, V currVal, boolean inherited) {
        ...
        if (inherited) {
            if (currVal == defVal) {
                valType = TextComponent.of(message("commands.region.flags.helper.inherited-default.prefix"))
                        .append(TextComponent.of(message("commands.region.flags.helper.default"))
                                .decoration(TextDecoration.UNDERLINED, true))
                        .append(TextComponent.of(message("commands.region.flags.helper.inherited-default.suffix")));
            } else {
                valType = TextComponent.of(message("commands.region.flags.helper.inherited"));
            }
        } else {
            if (currVal == defVal) {
                valType = TextComponent.empty()
                        .append(TextComponent.of(message("commands.region.flags.helper.default"))
                                .decoration(TextDecoration.UNDERLINED, true))
                        .append(TextComponent.of(message("commands.region.flags.helper.default-suffix")));
            } else {
                valType = null;
            }
        }
    ...
    private <V> void appendSetFlagValue(TextComponent.Builder builder, SetFlag<V> flag) {
        ...
        String display = (currVal == null ? "" : currVal.size() + message("commands.region.flags.helper.times")) + subName;
        ...
        if (currVal != null) {
            hoverComp = hoverComp.append(TextComponent.of(message("commands.region.flags.helper.current-values")))
    ...
    private void appendLocationFlagValue(TextComponent.Builder builder, LocationFlag flag) {
        ...
        if (currVal == null) {
            ...
            if (defVal == null) {
                appendValueText(builder, flag, message("commands.region.flags.helper.unset-location"), null);
            } else {
                appendValueText(builder, flag, defVal.toString(), TextComponent.of(message("commands.region.flags.helper.default-value"))
                        .append(TextComponent.newline()).append(TextComponent.of(defVal.toString())));
            }
        } else {
            appendValueText(builder, flag, currVal.toString(), TextComponent.of(message("commands.region.flags.helper.current-value"))
    ...
    private <V extends Number> void appendNumericFlagValue(TextComponent.Builder builder, NumberFlag<V> flag) {
        ...
        appendValueChoices(builder, flag, (Iterator<V>) choices.iterator(), choices.isEmpty() ? message("commands.region.flags.helper.unset-number") : message("commands.region.flags.helper.custom-label"));
    }
    ...
    private void appendStringFlagValue(TextComponent.Builder builder, StringFlag flag) {
        ...
        if (currVal == null) {
            ...
            if (defVal == null) {
                appendValueText(builder, flag, message("commands.region.flags.helper.unset-string"), null);
            } else {
                ...
                appendValueText(builder, flag, display, TextComponent.of(message("commands.region.flags.helper.default-value"))
            ...
            appendValueText(builder, flag, display, TextComponent.of(message("commands.region.flags.helper.current-value"))
